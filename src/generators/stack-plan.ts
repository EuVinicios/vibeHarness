...
export function stackPlanTemplate(input: StackPlanInput): string {
  ...
  // projectName comes from package.json — untrusted when the repo is cloned
  // from elsewhere; detectedStack labels derive from dependency names.
  const safeName = sanitizeInline(projectName);
  const safeStack = detectedStack.map((s) => sanitizeInline(s));
...
> Project type: **${projectType}** · Detected stack: ${safeStack.length ? safeStack.join(', ') : 'none'}
${resolvedBits.length ? `> ✅ **Already solved by your stack:** ${resolvedBits.join(' · ')} — no conflicting recommendations below.\n` : ''}${catalogStale ? '> ⚠️ **Registry is stale (> 30 days).** Run the registry sync before relying on these numbers.\n' : ''}`);
  ...
  sections.push(categorySection(catalog, 'Linting & Formatting', 'linting', 'Linter + formatter are dependency hygiene (Constitution Law 6) — they catch insecure patterns before the audit does.'));
